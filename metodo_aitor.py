...
# 1. CONFIGURACIÓN
st.set_page_config(page_title="A.I.T.O.R. 4.5 - EV TOTAL Focus", layout="wide")

# Inicializamos la tabla con nombres de columna fijos
...
# CALIDAD (LIBRO BLANCO) - IMPACTO DIRECTO EN EV Y IDT
...
# Plus de Calidad para el IDT (Máximo +35 pts)
plus_calidad_idt = p_eps_idt + (10 if c_inst else 0) + (10 if c_sector else 0)
# Plus para el EV TOTAL (Máximo +5.0 puntos de esperanza)
...
with tab1:
    ...
    # Suma: WR base + Bonus Calidad + Estructura + Señal - Penalización
    idt_total = wr_list[0] + plus_calidad_idt + p_estruc + p_senal - penal

    st.markdown("---")
    m1, m2, m3 = st.columns(3)
    with m1:
        # ETIQUETA CLARA DE EV TOTAL
    ...
    # LEYENDA
    with st.expander("📖 LEYENDA DE ESCALAS"):
        st.markdown("""
        **Clasificación de TIERS (Calidad del Activo)**
        * **👑 TIER S (EV Total ≥ 10):** Élite absoluta. Alta asimetría.
        * **🟢 TIER A (EV Total ≥ 5):** Guepardo operable.
        
        **Veredictos de Compra (Ejecución)**
        * **🔥 OBLIGATORIA:** IDT ≥ 100 y Riesgo ITE ≤ 5%.
        * **🟡 TÁCTICA:** IDT ≥ 85 y Riesgo ITE ≤ 8%.
        """)
    ...
    st.subheader("📋 Tu Ranking de Guepardos (Ordenado por EV TOTAL)")
    if not st.session_state.analisis.empty:
        # USAMOS EL NOMBRE EXACTO DE LA COLUMNA
